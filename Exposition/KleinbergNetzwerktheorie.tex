%!TEX root=./Emile.tex
\subsection{Menschliches Zusammenleben - eine Welt voller Verbindungen}
...
Um Netzwerke, Systeme von Verbindungen zwischen Entitäten, theoretisch betrachten und analysieren zu können, gibt es die Möglichkeit sie als Graph darzustellen \citep[vgl.]{Kleinberg-2009-oz}.
...
Genauso kann man auch Beziehungen in einer Lerngruppe wie einer Schulklasse oder andere Netzwerke graphisch darstellen \autoref{fig:graph1} (\autopageref{fig:graph1}).
...
Innerhalb der Struktur des Graphs kann es Besonderheiten, wie z.B. Komponente geben, einzelne Netzwerke, welche nicht über einen Pfad mit jedem anderen Knoten im Graph verbunden sind \autoref{fig:graphconnected} (\autopageref{fig:graphconnected}).
...
Sein Grundsatz sagt aus: "„Wer viel hat, dem wird viel gegeben; wer wenig hat, dem wird genommen“ \citep[105]{Zimpel2012}.
...
\emph{Das Normalisierungsprinzip} kann als Gegenstück des Matthäusprinzips betrachtet werden: ``Wer wenig hat, bekommt. Wer viel hat, gibt.'' \citep[12]{Zimpel2012}.
...
Es entsteht ein ``Fluss des Gebens und Nehmens'' \citep[125]{Zimpel2012}, bei dem, trotz der unterschiedlichen Voraussetzungen der Schüler, alle in den Lernprozess eingebunden werden konnten.
